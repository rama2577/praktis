...
| Storage dokumen | Filesystem `uploads/` (terenkripsi) | Object storage private (S3/R2/MinIO) — tetap enkripsi AES di atasnya | TD-04 |
| Secret | `.env` lokal | Secret manager (AWS Secrets Manager / Vault / Doppler) + rotasi | TD-05 |
| LLM | env langsung | API key via secret manager; fallback rule-engine tetap | TD-06 |
