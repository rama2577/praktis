...
| **F2.5B · Edit saat Review** ✅ (2026-08-11, PRD v2 M1) | Panel review bisa edit isi jurnal + audit trail + feedback ke KB | PRD v2 M1 → BUILD-PLAN-v2 M2 | ✅ `JournalCorrection` (field/before/after/accountCode/stage) + `POST /api/reviews/[taskId]/edit`; simpan = revisi draft (status TETAP di stage, task tetap PENDING); ActivityLog `JOURNAL_EDITED` + diff; insight A6 "Akun Paling Sering Dikoreksi" di quality page; UI editor baris (kode/akun/debit/kredit/keterangan, tambah/hapus baris, live balance); 13 test baru (204 total), lint 0, build OK; verifikasi live: koreksi 1:1 (debit 3.900.000→3.950.000, kredit 4.329.000→4.379.000; nama akun Kas→Kas & Bank) tercatat persis di JournalCorrection + ActivityLog; stage tidak berubah |
| **F2.5C · 9 Kategori Dokumen** ✅ (2026-08-11, PRD v2 M2) | Ekspansi DocumentType + mapping jenis→jenis jurnal | PRD v2 M2 → BUILD-PLAN-v2 M3 | ✅ commit e181603 — 9 kategori (Financial Statement, General Journal, Bank, Hutang/Pembelian, Piutang/Penjualan, Inventory, Gaji, Pajak, Lain-lain) + INVOICE/RECEIPT legacy; `doc-type-map.ts` (label + hint + preferensi); rule-engine pilih rule terbaik sesuai jenis; drafting LLM + hint; upload/portal form 9 opsi; test 222/222 |
| **F2.5D · Trial Balance** ✅ (2026-08-11, PRD v2 M3) | Agregasi saldo per akun/periode + indikator kewajaran + comparatif + ekspor | PRD v2 M3 → BUILD-PLAN-v2 M4 | ✅ commit c1c3bf9 — `trial-balance.ts` (parse period, klasifikasi COA 1–5xx, saldo normal, aset kredit/piutang negatif/liabilitas debit, komparatif bulan lalu); API `?period=&format=csv|xlsx`; halaman `/dashboard/reports/trial-balance` (filter klien+periode, kartu ringkas, tabel, ekspor); test 238/238; live: Maju Jaya 2026-08 total debit=kredit Rp 10.629.000 |
| **F2.5E · Buku Besar + Kunci Periode** ✅ (2026-08-11, PRD v2 M4/A5) | Drill-down akun→buku besar, reclass, FINALIZED + FiscalPeriod lock | PRD v2 M4/A5 → BUILD-PLAN-v2 M5 | ✅ commit cdd083c — model `FiscalPeriod` (OPEN/CLOSED) + status `FINALIZED`; buku besar saldo berjalan; `lockPeriod` (partner/admin) finalisasi jurnal APPROVED; `reclassJournal` (guard status/period/balance/COA, audit JOURNAL_EDITED); TB hitung APPROVED+FINALIZED + badge status + tombol kunci; halaman ledger + panel reclass; test 248/248; live: lock 2026-08 Maju Jaya → CLOSED + 3 FINALIZED, banner & reclass-off saat terkunci |
