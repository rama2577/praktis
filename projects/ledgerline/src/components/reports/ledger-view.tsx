  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <Link
          href={backHref}
          className="rounded-lg border border-slate-700 px-3 py-2 text-sm text-slate-300 transition hover:border-yellow-400/50 hover:text-yellow-300"
        >
              ...
              <TBody>
                {report.entries.map((e) => (
                  <TR key={e.journalId}>
                    ...
                    <TD className="text-right font-mono text-slate-100">{formatCurrencyRp(e.balance)}</TD>
                    {canEdit && report.status === "OPEN" && (
                      <TD className="text-right">
                        <button
                          type="button"
                          onClick={() => void startEdit(e)}
                          className="rounded border border-slate-700 px-2 py-1 text-xs text-slate-300 transition hover:border-yellow-400/50 hover:text-yellow-300"
                        >
                          Reclass
                        </button>
                      </TD>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className="max-h-[85vh] w-full max-w-2xl overflow-y-auto rounded-xl border border-slate-700 bg-slate-900 p-5">
            <div className="mb-4 flex items-center justify-between">
              <div>
                <h3 className="text-sm font-medium text-slate-100">Reclass Jurnal</h3>
              ...
              {editLines.map((l, i) => (
                <div key={i} className="grid grid-cols-12 gap-2 text-sm">
                  <input
                    ...
                    className="col-span-2 rounded border border-slate-700 bg-slate-950 px-2 py-1.5 text-slate-200 focus:border-yellow-400/50 focus:outline-none"
                  />
                  <input
                    value={l.accountName}
                    onChange={(e) => updateLine(i, { accountName: e.target.value })}
                    placeholder="Nama akun"
                    className="col-span-4 rounded border border-slate-700 bg-slate-950 px-2 py-1.5 text-slate-200 focus:border-yellow-400/50 focus:outline-none"
                  />
                  <input
                    value={l.debit}
                    ...
                    placeholder="Debit"
                    inputMode="decimal"
                    className="col-span-2 rounded border border-slate-700 bg-slate-950 px-2 py-1.5 text-right font-mono text-slate-200 focus:border-yellow-400/50 focus:outline-none"
                  />
                  <input
                    value={l.credit}
                  ...
                  <button
                    ...
                    className="col-span-2 rounded border border-slate-800 px-2 py-1.5 text-xs text-slate-500 hover:text-red-300 disabled:opacity-30"
                  >
                    Hapus
                  </button>
                </div>
            ...
            <div className="mt-3 flex items-center justify-between text-sm">
              <button
                type="button"
                onClick={addLine}
                className="rounded border border-slate-700 px-2 py-1.5 text-xs text-slate-300 hover:border-yellow-400/50 hover:text-yellow-300"
              >
                + Tambah baris
              </button>
              <span className={balanced ? "text-emerald-300" : "text-red-300"}>
                {balanced
