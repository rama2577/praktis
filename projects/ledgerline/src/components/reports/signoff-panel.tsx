...
export function SignoffPanel({
  ...
  if (loading) return <div className="text-xs text-slate-600">Memuat status…</div>;
  ...
  if (!snapshot) {
    return (
      <div className="flex items-center gap-2 rounded-lg border border-slate-200 bg-slate-100/30 px-3 py-2">
        ...
        <span className="text-xs text-slate-600">Simpan laporan untuk mulai workflow</span>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-2">
      <div className="flex flex-wrap items-center gap-2">
        ...
        {snapshot.reviewedBy && (
          <span className="text-xs text-slate-600">Review: {snapshot.reviewedBy}</span>
        )}
        {snapshot.approvedBy && (
          <span className="text-xs text-slate-600">Approval: {snapshot.approvedBy}</span>
        )}
      </div>
