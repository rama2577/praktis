...
export function SignoffPanel({
  ...
  if (loading) return <div className="text-xs text-slate-500">Memuat status…</div>;
  ...
  return (
    <div className="space-y-2">
      <div className="flex flex-wrap items-center gap-2">
        ...
        {snapshot.reviewedBy && (
          <span className="text-xs text-slate-500">Review: {snapshot.reviewedBy}</span>
        )}
        {snapshot.approvedBy && (
          <span className="text-xs text-slate-500">Approval: {snapshot.approvedBy}</span>
        )}
      </div>
