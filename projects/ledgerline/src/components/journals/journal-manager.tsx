...
export function JournalManager({ canWrite }: { canWrite: boolean }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {canWrite && (
        <div>
          <button
            ...
            className="rounded-lg bg-accent px-4 py-2 text-sm font-semibold text-[#0b1120] transition hover:bg-[#ffdf6b]"
          >
          ...
          {showForm && (
            ...
            >
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <label className="block text-sm">
                  <span className="mb-1 block text-slate-400">Tanggal transaksi *</span>
                  <input
                    type="date"
                    value={entryDate}
              ...
              <div className="mt-5">
                <div className="mb-2 flex items-center justify-between">
                  <p className="text-sm text-slate-400">Baris jurnal (debit = kredit)</p>
                  <button
                    type="button"
                    onClick={() => setLines((p) => [...p, emptyLine()])}
                    className="rounded-lg border border-line px-3 py-1.5 text-xs text-slate-300 transition hover:border-accent/50 hover:text-accent"
                  >
                    + Tambah Baris
                  </button>
                </div>
                  ...
                  <table className="w-full min-w-[640px] text-sm">
                        ...
                        <td className="px-3 py-1.5">
                          ...
                          <td className="px-3 py-1.5">
                            <input
                              ...
                              placeholder="0"
                              className="w-28 rounded border border-line bg-[#0b1120] px-2 py-1.5 text-right font-mono text-xs text-slate-200 focus:border-accent/60 focus:outline-none"
                          ...
                          <td className="px-3 py-1.5">
                            <button
                              ...
                              className="rounded border border-line px-2 py-1 text-xs text-slate-500 transition hover:border-red-500/40 hover:text-red-400 disabled:opacity-40"
                    ...
                    <tfoot>
                      <tr className="border-t border-line bg-white/[0.02]">
                        <td colSpan={2} className="px-3 py-2 text-xs font-semibold text-slate-400">
                          Total
                        </td>
                        <td className="px-3 py-2 text-right font-mono text-xs text-slate-200">{formatCurrencyRp(totals.debit)}</td>
                        <td className="px-3 py-2 text-right font-mono text-xs text-slate-200">{formatCurrencyRp(totals.credit)}</td>
                        <td colSpan={2} className="px-3 py-2">
                          {totals.debit > 0 && (
                            <span
                              className={`rounded px-2 py-0.5 text-[11px] font-semibold ${
                                totals.balanced ? "bg-emerald-500/15 text-emerald-300" : "bg-red-500/15 text-red-300"
                              }`}
                            >
              ...
              <div className="mt-5 flex items-center gap-3">
                <button
                  type="submit"
                  disabled={busy}
                  className="rounded-lg bg-accent px-5 py-2.5 text-sm font-semibold text-[#0b1120] transition hover:bg-[#ffdf6b] disabled:opacity-50"
                >
      ...
      <div>
        <h2 className="mb-3 text-sm font-semibold text-slate-200">Daftar Jurnal Manual & Penyesuaian</h2>
        {loading ? (
              ...
              <tbody>
                {journals.map((j) => (
                  <tr key={j.id} className="border-b border-line/50 last:border-0 hover:bg-white/[0.02]">
                    ...
                    <td className="px-4 py-3 text-slate-200">{j.client.name}</td>
                    <td className="max-w-[260px] truncate px-4 py-3 text-slate-300">{j.description}</td>
                    ...
                    <td className="px-4 py-3 text-right font-mono text-xs text-slate-400">{j.lines.length}</td>
                  </tr>
                ))}
              </tbody>
            </table>
