...
const VO = ROOT + "/vo-rama";
const SLIDES = ROOT + "/assets";
const BUILD = ROOT + "/build3";
...
const seg = {};
for (const s of ["seg1", "seg2", "seg3", "seg4", "seg5", "seg6"]) seg[s] = dur(`${VO}/${s}.wav`);
console.log("vo(rama):", seg);

const r = n => Math.round(n * 10) / 10;
const introDur = r(seg.seg1 + 1.0);
const dashCap = r(seg.seg2 + 0.8);
const pipeCap = r(seg.seg3 + 0.8);
const revCap = r(seg.seg4 + 0.8);
const custCap = r(seg.seg5 + 0.8);
const outroDur = r(seg.seg6 + 1.5);
...
// normalize VO to 44.1k mono (already wav, keep consistent)
for (const s of ["seg1", "seg2", "seg3", "seg4", "seg5", "seg6"]) {
  sh(`ffmpeg -y -v error -i "${VO}/${s}.wav" -ar 44100 -ac 1 "${BUILD}/${s}.wav"`);
}

const S1 = 0, S2 = introDur, S3 = S2 + dashCap, S4 = S3 + pipeCap, S5 = S4 + revCap, S6 = S5 + custCap;
const off = [
  Math.round((S1 + 0.3) * 1000),
  Math.round((S2 + 0.5) * 1000),
  Math.round((S3 + 0.4) * 1000),
  Math.round((S4 + 0.4) * 1000),
  Math.round((S5 + 0.4) * 1000),
  Math.round((S6 + 0.3) * 1000),
];
...
sh(`ffmpeg -y -v error -i "${BUILD}/full.mp4" -i "${BUILD}/audio-mix.wav" -map 0:v -map 1:a -c:v copy -c:a aac -b:a 192k -shortest "${ROOT}/video-praktis-v3.mp4"`);

console.log("FINAL", dur(`${ROOT}/video-praktis-v3.mp4`), "s");
